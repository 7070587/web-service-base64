export class ServiceBase64Class {
    /**
     * is base64 image
     * @param  {string} value
     * @returns boolean
     */
    public isBase64Image(value: string): boolean {
        let base64HeaderRegex: RegExp = /data:image\/([a-zA-Z]*);base64,([^\"]*)/;
    ...
    public isUrl(value: string): boolean {
        let urlRegex: RegExp = /^(ws|wss|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/i;
     ...
     * @returns Promise
     */
    public async getBase64(file: File): Promise<FileReader> {
     ...
     * @returns Promise
     */
    public async fileToBase64(file: File, fileSize: number = null): Promise<FileReader> {
        let result: FileReader;
        if (file) {
     ...
     * @returns Promise
     */
    public async urlImageToBase64(url: string): Promise<string> {
        let result: string;
        if (this.isUrl(url)) {
            result = await new Promise((resolve, reject) => {
                try {
     ...
     * @returns string
     */
    public getBase64ContentType(base64: string): string {
        let parts = base64.split(';base64,');
        let contentType = parts[0].split(':')[1];
        return contentType;
    }
     ...
     * @returns string
     */
    public getBase64String(base64: string): string {
        let parts = base64.split(';base64,');
     ...
     * @returns string
     */
    public getBase64FullString(contentType: string, base64: string): string {
     ...
     * @returns string
     */
    public imageBase64Src(base64: string, fileType: TImageFileType = EImageFileType.jpg): string {
